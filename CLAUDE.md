...
- **`interpretDurable`'s leaf/sink cases now thread `LeafOpts.heavy`/`memo` (not just
  `retries`, #1071) into `runStep`'s memo-cache short-circuit and heavyConcurrency gate —
  see `op-engine/durable.ts` and `op-engine/caps.ts`'s `makeGovernors`/`kvCache` for the
  Workflows-runtime wiring that backs it (#1157).**
