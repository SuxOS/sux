...
| `ingest` | Capture into the vault: one of `url` (HTML → markdown; binaries become attachments) \| `text` \| `query` (search results) → a provenance-stamped note in `Inbox/`. Blobs ≤1MB commit into the vault repo; larger (or `blobs:'dropbox'`) upload to the Dropbox app folder and the note links the shared URL (R2 fallback when `DROPBOX_TOKEN` unset). |
| `dropbox` | Dropbox app-folder blob store (`DROPBOX_TOKEN`, App-folder scoped — can't see the rest of Dropbox): put/get/list/delete/share; put returns a shared link. The human-facing twin of R2 `store` — files sync to every device. |
