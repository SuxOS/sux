...
describe("vaultAuthHeaders — suxbot App configured", () => {
	...
	it("mints and prefers the installation token over the PAT for vault traffic", async () => {
		const env = appEnv();
	...
	it("scopes the minted token to JUST the vault repo (least-privilege, derived from OBSIDIAN_VAULT_REPO)", async () => {
		await vaultAuthHeaders(appEnv(), VAULT_URL);
		// 'SuxOS/vault' → repository name 'vault' in the POST body, NOT an org-wide (empty-body) token.
		expect(JSON.parse(lastMint!.init.body)).toEqual({ repositories: ["vault"] });
	});

	it("derives the scope from the target — a different OBSIDIAN_VAULT_REPO narrows to that repo", async () => {
		const env = { ...appEnv(), OBSIDIAN_VAULT_REPO: "SuxOS/notes" };
		await vaultAuthHeaders(env, VAULT_URL);
		expect(JSON.parse(lastMint!.init.body)).toEqual({ repositories: ["notes"] });
		// Cache key carries the scope so a different repo can't reuse another's token.
		expect(env.OAUTH_KV.map.get("cache:suxbot:installation-token:notes")).toBeTruthy();
		expect(env.OAUTH_KV.map.get("cache:suxbot:installation-token:vault")).toBeFalsy();
	});

	it("mints nothing (falls back to the PAT) when the target repo is unset/malformed", async () => {
		expect(await vaultAuthHeaders({ ...appEnv(), OBSIDIAN_VAULT_REPO: undefined }, VAULT_URL)).toEqual({ Authorization: "Bearer pat-fallback" });
		expect(mints).toBe(0);
		expect(await vaultAuthHeaders({ ...appEnv(), OBSIDIAN_VAULT_REPO: "notaslug" }, VAULT_URL)).toEqual({ Authorization: "Bearer pat-fallback" });
		expect(mints).toBe(0);
	});
	...
	it("caches the token in KV and reuses it (no second mint)", async () => {
		...
		expect(mints).toBe(1);
		expect(env.OAUTH_KV.map.get("cache:suxbot:installation-token:vault")).toBeTruthy();
	});
