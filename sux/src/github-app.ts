...
// KV key PREFIX for the cached installation token; the scoped repo name is appended
// (`…:vault`) so a scope change never reuses a token minted for a different repo.
...
/**
 * The repo NAME (within the App's install account) to scope the minted token to,
 * derived from OBSIDIAN_VAULT_REPO ('SuxOS/vault' → 'vault'). Deriving it — rather
 * than hardcoding — keeps the scope tracking the actual write target if it ever
 * moves. Null when the target is unset/malformed, in which case we mint NOTHING
 * (the caller falls back to GITHUB_TOKEN): a token is never minted broader than the
 * one repo the vault backend writes, and vault ops can't run without this var anyway.
 */
export function vaultRepoName(env: SuxbotEnv): string | null {
...
export async function suxbotInstallationToken(env: SuxbotEnv): Promise<string | null> {
	...
	// Least-privilege: mint a token scoped to JUST the vault repo, never the App's
	// org-wide install. No derivable target ⇒ mint nothing (fall back to the PAT).
	const repoName = vaultRepoName(env);
	if (!repoName) return null;
	// Cache key carries the repo scope, so a re-scope (target moves) never serves a
	// stale token minted for a different/broader repo.
	const cacheKey = `${TOKEN_KEY}:${repoName}`;
	try {
		...
		const resp = await smartFetch(env, `${GH}/app/installations/${encodeURIComponent(String(env.SUX_BOT_INSTALLATION_ID))}/access_tokens`, {
			...
			// `repositories: [<name>]` narrows the token to repository_selection:"selected"
			// on this one repo — a leaked token can touch only the vault, not the whole org.
