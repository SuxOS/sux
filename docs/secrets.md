...
- `MAIL_TRIAGE_ENABLED` → classify + suggest + digest; `MAIL_TRIAGE_ACT` → also do reversible moves (never delete/send). `MAIL_TRIAGE_ACT_OPS` (optional) → a comma-separated subset of the auto-act allow-list (`label:add`/`archive`/`unarchive`/`undelete`/`draft-reply`), INTERSECTED with it so it can only narrow, never widen, what may act — lets the rollout be ramped op-by-op; unset arms the full list, an explicitly empty/unparseable value fails closed (no ops act).
- `SELF_IMPROVE_ENABLE` → loop runs; `SELF_IMPROVE_PR` → may open PRs (needs `GITHUB_TOKEN`); `SELF_IMPROVE_REPO` → target; `SELF_IMPROVE_AUTOMERGE` → its own auto-merge (**leave OFF** — the GitHub `automerge.yml` pipeline does merging); `SELF_IMPROVE_KILL` → hard stop. (`SELF_IMPROVE_ARM` was the pre-#67 name for this flag — renamed to `SELF_IMPROVE_AUTOMERGE` in code; don't re-add the old name.)
